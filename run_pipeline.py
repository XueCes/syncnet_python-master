...
parser.add_argument('--data_root',  type=str,  default='F:\\AIGC\\111', help='')
parser.add_argument('--deviceid',  type=str, default='0', help='')
...
def crop_video(opt,track,cropfile):
  ...
  command = ("ffmpeg -y -i %st.avi -i %s -c:v libx264 -crf 1 -c:a aac %s.mp4" % (cropfile, audiotmp, cropfile))
...
def inference_video(opt):
  ...
  DET = S3FD(device='cuda:{}'.format(opt.deviceid))
...
face_list = os.listdir(opt.data_root)
for face in face_list:
  video_list = os.listdir(os.path.join(opt.data_root, face))
  os.mkdir(os.path.join(opt.crop_dir, opt.reference, face))
  for jj, video in enumerate(video_list):
    ...
    command = ("ffmpeg -y -i %s -qscale:v 2 -async 1 -r 25 %s" % (os.path.join(opt.data_root, face, video),os.path.join(opt.avi_dir,opt.reference,'video.avi')))
    output = subprocess.call(command, shell=True, stdout=None)
    ...
    for ii, track in enumerate(alltracks):
      vidtracks.append(crop_video(opt,track,os.path.join(opt.crop_dir,opt.reference,face,'%05d'%jj)))
